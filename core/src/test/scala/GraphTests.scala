...
object GraphTests extends Properties("Graph") {
  ...
  property("embed and decomp are inverses of each other") = forAll {
    (g: Graph[N, Int, Int], c: Context[N, Int, Int]) =>
  ...
  property(
    "Adding a node and removing it again yields a graph that doesn't contain that node"
  ) = forAll { (g: Graph[N, Int, Int], n: LNode[N, Int]) =>
  ...
  property(
    "Adding an edge and removing it again yields a graph that doesn't contain that edge"
  ) = forAll { (g: Graph[N, Int, Int], e: LEdge[N, Int]) =>
    g.addNode(LNode(e.from, 0))
      .addNode(LNode(e.to, 0))
      .addEdge(e)
  ...
  property(
    "A graph constructed from nodes and edges should contain those nodes and edges"
    ...
    val ees = es.toSet.toList
      .filter((x: LEdge[N, Int]) => ns.contains(x.from) && ns.contains(x.to))
      .sorted
  ...
  property(
    "A graph constructed from another graph's nodes and edges should be the same graph"
  ...
  property("Adding an edge between two existing nodes should succeed") =
    forAll { (n1: LNode[N, String], n2: LNode[N, String], el: String) =>
      !empty[N, String, String]
        .addNode(n1)
        .addNode(n2)
        .addEdge(LEdge(n1.vertex, n2.vertex, el))
        .isEmpty
    }

  property("DFS should find all descendants of a node") = forAll {
    (ns: List[LNode[N, Int]], l: Int) =>
      val g = connect(ns, l)
      ns.isEmpty || g.dfs(Seq(ns.head.vertex)).toSet == ns.map(_.vertex).toSet
  }

  property("RDFS should find all ancestors of a node") = forAll {
    (ns: List[LNode[N, Int]], l: Int) =>
      val g = connect(ns, l)
      ns.isEmpty || g.rdfs(Seq(ns.last.vertex)).toSet == ns.map(_.vertex).toSet
  }

  property("BFS should find all descendants of a node") = forAll {
    (ns: List[LNode[N, Int]], l: Int) =>
      val g = connect(ns, l)
  ...
  property(
    "The union of two graphs should contain all the edges and nodes of both graphs"
  ...
  property("Union is commutative (ignoring labels)") = forAll {
    (g1: Graph[N, Int, Int], g2: Graph[N, Int, Int]) =>
      val u1                            = g1 union g2
      val u2                            = g2 union g1
  ...
  property("Folding on contexts equals folding on nodes and edges") = forAll {
    (g: Graph[N, Int, Int]) =>
      (g.labNodes.map(_.label).sum, g.labEdges.map(_.label).sum) == g.fold(
        (0, 0)
      ) {
  ...
  property("The roots of a graph have no incoming edges from other nodes") =
    forAll { (ns: List[LNode[N, Int]], es: List[LEdge[N, Int]]) =>
      val g  = safeMkGraph(ns, es)
      val vs = ns.map(_.vertex).toSet
      val roots = vs.filterNot(n =>
        es.find { e =>
  ...
  property("The leaves of a graph have no outgoing edges to other nodes") =
    forAll { (ns: List[LNode[N, Int]], es: List[LEdge[N, Int]]) =>
      val g  = safeMkGraph(ns, es)
      val vs = ns.map(_.vertex).toSet
      val leaves = vs.filterNot(n =>
        es.find { e =>
  ...
  property("The shortest path should be the cheapest path with constant cost") =
    forAll { (tpg: (Graph[N, Int, Int], LNode[N, Int], LNode[N, Int])) =>
      val graph = tpg._1
      val from  = tpg._2.vertex
      val to    = tpg._3.vertex
      val sPath = graph.lesp(from, to)
      val cPath = graph.cheapestPath[Int](
        from,
        to,
        (_: LNode[N, Int], _: Int, _: LNode[N, Int]) => 1
      )
  ...
  implicit def cogenGrContext[V: Cogen: Ordering, A: Cogen, B: Cogen: Ordering]
      : Cogen[GrContext[V, A, B]] =
    Cogen[(Map[V, Set[B]], A, Map[V, Set[B]])].contramap(c =>
      (c.inAdj, c.label, c.outAdj)
    )

  implicit def cogenContext[V: Cogen: Ordering, A: Cogen, B: Cogen: Ordering]
      : Cogen[Context[V, A, B]] =
    Cogen[(V, GrContext[V, A, B])].contramap(c => c.vertex -> c.toGrContext)

  implicit def cogenGDecomp[V: Cogen: Ordering, A: Cogen, B: Cogen: Ordering]
      : Cogen[GDecomp[V, A, B]] =
    Cogen[(Context[V, A, B], Graph[V, A, B])].contramap(d => (d.ctx, d.rest))

  implicit def cogenGraph[V: Cogen: Ordering, A: Cogen, B: Cogen: Ordering]
      : Cogen[Graph[V, A, B]] =
    Cogen[Map[V, GrContext[V, A, B]]].contramap(_.rep)

  include(
    ComonadTests[({ type λ[α] = GDecomp[N, α, Int] })#λ]
      .comonad[Int, Int, Int]
      .all
  )

  property(
    "The shortest path through a graph should be the shortest path between all subpaths"
  ) = forAll { (tpg: (Graph[N, Int, Int], LNode[N, Int], LNode[N, Int])) =>
    val start = tpg._2
    val end   = tpg._3
    val graph = tpg._1
    ...
    } yield shortest.nonEmpty && shortest.get.nonEmpty && shortest.get.get == subpath)
      .forall(identity)
  }

  property(
    "The shortest labelled path through a graph should be labelled by the labelled shortest labelled path between all subpaths"
  ) = forAll { (tpg: (Graph[N, Int, Int], LNode[N, Int], LNode[N, Int])) =>
    val start = tpg._2
    val end   = tpg._3
    val graph = tpg._1
    (for {
      ...
      subpath <- (0 until path._2.size)
        .map(n => (path._1, path._2.dropRight(n)))
        .toVector
      shortest =
        graph.esp(subpath._1, subpath._2.lastOption.fold(subpath._1)(_._1))
    } yield shortest.nonEmpty && shortest.get.length == (subpath._2.length + 1))
      .forall(identity)
  }

  property(
    "The shortest labelled path through a graph without the labels should be the shortest path exactly"
  ) = forAll { (tpg: (Graph[N, Int, Int], LNode[N, Int], LNode[N, Int])) =>
    val start = tpg._2
    val end   = tpg._3
    val graph = tpg._1
    ...
    s"Labelled path is $lpath which should be $path if the labels are dropped" |: lpath
      .map { case (s, p) => s +: p.map(_._1) } === path
  }
}
