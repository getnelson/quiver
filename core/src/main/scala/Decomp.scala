...
/**
  ...
  */
case class Decomp[N, A, B](
    ctx: Option[Context[N, A, B]],
    rest: Graph[N, A, B]
) {
...
case class GDecomp[N, A, B](ctx: Context[N, A, B], rest: Graph[N, A, B]) {
  def addSucc(node: LNode[N, A], edge: B): GDecomp[N, A, B] =
    GDecomp(
      Context(Vector(edge -> ctx.vertex), node.vertex, node.label, Vector()),
      rest & ctx
    )
  def addPred(node: LNode[N, A], edge: B): GDecomp[N, A, B] =
    GDecomp(
      Context(Vector(), node.vertex, node.label, Vector(edge -> ctx.vertex)),
      rest & ctx
    )
  ...
  def redecorate[C](f: GDecomp[N, A, B] => C): GDecomp[N, C, B] =
    GDecomp(
      ctx.copy(label = f(this)),
      rest.gmap { c =>
        c.copy(label = f(toGraph.decomp(c.vertex).toGDecomp.get))
      }
    )
...
object GDecomp {
  ...
  implicit def gDecompComonad[N, B]
      : Comonad[({ type λ[α] = GDecomp[N, α, B] })#λ] =
    new Comonad[({ type λ[α] = GDecomp[N, α, B] })#λ] {
...
case class BiDecomp[N, A, B](
  ...
  def addSucc(node: LNode[N, A], edge: B): BiDecomp[N, A, B] =
    BiDecomp(
      first,
      Context(Vector(edge -> last.vertex), node.vertex, node.label, Vector()),
      rest & last
    )
  ...
  def addPred(node: LNode[N, A], edge: B): BiDecomp[N, A, B] =
    BiDecomp(
      Context(Vector(edge -> first.vertex), node.vertex, node.label, Vector()),
      last,
      rest & first
    )
  ...
  def append(b: BiDecomp[N, A, B], edge: B): BiDecomp[N, A, B] =
    BiDecomp(
      first,
      b.last,
      rest union b.rest & last & b.first addEdge LEdge(
        last.vertex,
        b.first.vertex,
        edge
      )
