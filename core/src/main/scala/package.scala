...
package object quiver {
  ...
  /** @group instances */
  implicit def nodeOrder[N, A](implicit
      N: Order[N],
      A: Order[A]
  ): Order[LNode[N, A]] =
    Order.whenEqual(
  ...
  /** @group instances */
  implicit def ledgeOrder[N, A](implicit
      N: Order[N],
      A: Order[A]
  ): Order[LEdge[N, A]] =
    Order.whenEqualMonoid.combineAll(
      List(Order.by(_.from), Order.by(_.to), Order.by(_.label))
    )

  /** @group instances */
  implicit def edgeOrder[N, A](implicit N: Order[N]): Order[Edge[N]] =
    Order.whenEqualMonoid.combineAll(List(Order.by(_.from), Order.by(_.to)))

  /** @group instances */
  implicit def graphOrder[N, A, B](implicit
      N: Order[N],
      A: Order[A],
      B: Order[B]
  ): Order[Graph[N, A, B]] = {
    implicit val L: Ordering[LNode[N, A]] = Order[LNode[N, A]].toOrdering
    implicit val E: Ordering[LEdge[N, B]] = Order[LEdge[N, B]].toOrdering
    Order.whenEqual(Order.by(_.labNodes.sorted), Order.by(_.labEdges.sorted))
  }

  implicit def contextOrder[N, A, B](implicit
      N: Order[N],
      A: Order[A],
      B: Order[B]
  ): Order[Context[N, A, B]] = {
    implicit val adj: Ordering[(B, N)] = Order[(B, N)].toOrdering
    Order.whenEqualMonoid.combineAll(
      List(
        Order.by(_.vertex),
        Order.by(_.label),
  ...
  implicit def gDecompOrder[N: Order, A: Order, B: Order]
      : Order[GDecomp[N, A, B]] =
    Order.whenEqual(
  ...
  def star(n: Int): Graph[Int, Unit, Unit] =
    mkGraph(
      Range(0, n).map(LNode(_, ())),
      Range(1, n).map(v => LEdge(0, v, ()))
    )
    ...
    Range(0, n)
      .map { n =>
        ...
        star(k)
          .vmap(_ + j)
          .addNode(LNode(0, ()))
          .addEdge(LEdge(0, n * k + 2, ()))
      }
      .foldLeft(empty[Int, Unit, Unit])(_ union _)
      .undir
  ...
  def clear[N, A, B](
      ...
      v: N,
      ns: Vector[N],
      f: GrContext[N, A, B] => GrContext[N, A, B]
  ): GraphRep[N, A, B] =
  ...
  def addSucc[N, A, B](
    ...
    if (lps.isEmpty) g
      ...
      addSucc(
        g.alter(lps.head._2)(_ map { (x: GrContext[N, A, B]) =>
          x match {
  ...
  def addPred[N, A, B](
    ...
    if (lss.isEmpty) g
      ...
      addPred(
        g.alter(lss.head._2)(_ map { (x: GrContext[N, A, B]) =>
          x match {
            case GrContext(ps, lp, ss) =>
  ...
  def reverseLPath[N, A](p: LPath[N, A]): LPath[N, A] =
    p match {
      ...
      case (s, op) =>
        (
          op.last._1,
          op.foldLeft[(N, Vector[(N, A)])]((s, Vector.empty[(N, A)])) {
  ...
  private[quiver] implicit class MapOps[A, B](val self: Map[A, B])
      extends AnyVal {
