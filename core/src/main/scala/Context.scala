...
case class Context[N, A, B](
  ...
  def toGrContext: GrContext[N, A, B] =
    GrContext(fromAdj(inAdj), label, fromAdj(outAdj))
...
case class GrContext[N, A, B](
    inAdj: Map[N, Set[B]],
    label: A,
    outAdj: Map[N, Set[B]]
) {
  def toContext(v: N): Context[N, A, B] =
    Context(toAdj(inAdj), v, label, toAdj(outAdj))
}
