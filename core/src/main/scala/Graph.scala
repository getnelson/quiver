...
case class Graph[N, A, B](rep: GraphRep[N, A, B]) {
  ...
  def addEdge(e: LEdge[N, B]): Graph[N, A, B] =
    safeAddEdge(
      e,
      sys.error(
        s"Can't add edge $e since the source and target nodes don't both exist in the graph."
      )
    )
  ...
  /**
    ...
    * @group mutation
    */
  def removeEdge(e: Edge[N]): Graph[N, A, B] =
    decomp(e.from) match {
  ...
  /**
    ...
    * @group mutation
    */
  def removeLEdge(e: LEdge[N, B]): Graph[N, A, B] =
    decomp(e.from) match {
      case Decomp(None, _) => this
      case Decomp(Some(Context(p, v, l, s)), gp) =>
        gp & Context(
          ...
          v,
          l,
          s.filter { case (x, n) => (x != e.label) || (n != e.to) }
        )
    }
  ...
  def labNodes: Vector[LNode[N, A]] =
    rep.toVector map {
      case (node, GrContext(_, label, _)) => LNode(node, label)
    }
  ...
  /**
    ...
    * @group projection
    */
  def edges: Vector[Edge[N]] =
    labEdges.map { case LEdge(v, w, _) => Edge(v, w) }
  ...
  def nmap[C](f: A => C): Graph[N, C, B] =
    Graph(rep.map {
      case (k, GrContext(ps, a, ss)) => k -> GrContext(ps, f(a), ss)
    })
  ...
  def emap[C](f: B => C): Graph[N, A, C] =
    Graph(rep.map {
      case (k, GrContext(ps, a, ss)) =>
        k -> GrContext(
          ps map { case (k, v) => k -> (v map f) },
          a,
          ss map { case (k, v) => k -> (v map f) }
        )
    })
  ...
  def vmap[M](f: N => M): Graph[M, A, B] =
    Graph(rep.map {
      case (k, GrContext(ps, a, ss)) =>
        f(k) -> GrContext(ps mapKeys f, a, ss mapKeys f)
    })
  ...
  /**
    ...
    */
  def filterMap[C, D](
      f: Context[N, A, B] => Option[Context[N, C, D]]
  ): Graph[N, C, D] =
    ...
    if (vs.isEmpty || isEmpty) (Vector(), this)
      ...
      decomp(vs.head) match {
        ...
        case Decomp(Some(c), g) =>
          ...
          (Node(f(c), LazyList.from(xs)) +: ys, g3)
      }
  ...
  def bf(q: Queue[Path[N]]): RTree[N] =
    if (q.isEmpty || isEmpty) LazyList.empty
    else {
      val (p, qp) = q.dequeue
      if (p.isEmpty) LazyList(p)
        ...
        decomp(p.head) match {
          case Decomp(Some(c), g) =>
            p #:: g.bf(qp.enqueueAll(c.successors.map(_ +: p)))
  ...
  def lbf(q: Queue[LPath[N, B]]): LRTree[N, B] =
    if (q.isEmpty || isEmpty) LazyList.empty
      ...
      decomp(p._1) match {
        case Decomp(Some(c), g) =>
          p #:: g.lbf(
            qp.enqueueAll(c.outs.map(nn => (nn._2, (p._1, nn._1) +: p._2)))
          )
  ...
  /**
    ...
    * @group bfs
    */
  def cheapestPath[C: Monoid: math.Ordering](
      s: N,
      t: N,
      costFkt: (LNode[N, A], B, LNode[N, A]) => C
  ): Option[LPath[N, B]] = {
    def costOfPath(p: LPath[N, B]): C =
      p._2
        .foldLeft((LNode(p._1, label(p._1).get), Monoid[C].empty)) {
